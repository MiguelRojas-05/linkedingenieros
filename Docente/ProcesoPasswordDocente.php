...
if (strcmp($Password,$Registro["password"])==0) {
    ...
    if (strcmp($NuevoPassword,$RePassword)==0) {
        ...
        if(strlen($NuevoPassword)>=8 && strlen($RePassword)>=8){
            ...
            header('location:ActualizarPasswordDocente.php?e=inc');
        }
    }else{
        //Redireccionamos a
        header('location: ActualizarPasswordDocente.php?e2=inc');
    }
}else{
    header('location: ActualizarPasswordDocente.php?e3=inc');
}
?>